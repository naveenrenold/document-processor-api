...
namespace DocumentProcessor.Endpoints
{
    public static class AttachmentEndpoint
    {
        ...
        public static async Task<IResult> GetAttachment(IAttachmentDL attachmentDL, [AsParameters] QueryFilter filter)
        {
            var response = await attachmentDL.GetAttachment(filter);
            return Results.Ok(response);
        }
        public static async Task<IResult> PostAttachment(IAttachmentDL attachmentDL, [AsParameters] QueryFilter filter)
        {
            var response = await attachmentDL.GetAttachment(filter);
            return Results.Ok(response);
        }
